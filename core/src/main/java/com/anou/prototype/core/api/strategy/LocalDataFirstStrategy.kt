    ...
    private fun askRemote(warning: Throwable, data: T? = null) = mainScope.launch {
        if (isRemoteAvailable()) {
            try {
                ...
                val data = task.await()
                liveData.postValue(ResourceWrapper(value = data, status = ResourceStatus.SUCCESS, localData = false, warning = warning))
            ...
            liveData.postValue(ResourceWrapper(value = data, error = IllegalStateException("Remote value is not available"), status = ResourceStatus.INVALID, localData = false, warning = warning))
        }
    }
